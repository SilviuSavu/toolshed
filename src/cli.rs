...
pub enum Command {
    List { category: Option<String>, health: bool },
    Help { tool: String, command: Option<String> },
    Run { tool: String, command: String, full: bool, timeout: Option<u64>, args: Vec<String> },
    Status,
    Stop { tool: Option<String> },
    Validate { tool: Option<String> },
    AgentPrompt { format: String },
    Skill { action: SkillAction },
    Agent { action: AgentAction },
    Rule { action: RuleAction },
    Workflow { action: WorkflowAction },
    Serve { port: u16, category: Option<String> },
...
pub enum WorkflowAction {
    ...
    Show { name: String },
    Validate { name: Option<String> },
    Run { name: String, full: bool, timeout: Option<u64>, verbose: bool },
}
...
fn workflow_subcommand() -> clap::Command {
    clap::Command::new("workflow")
        ...
        .subcommand(clap::Command::new("show").about("Show a workflow's steps").arg(Arg::new("name").required(true)))
        .subcommand(clap::Command::new("validate").about("Validate workflow manifests").arg(Arg::new("name")))
        .subcommand(
            clap::Command::new("run")
                .about("Run a workflow")
                .arg(Arg::new("name").required(true))
                .arg(Arg::new("full").long("full").action(clap::ArgAction::SetTrue))
                .arg(Arg::new("timeout").long("timeout").value_parser(clap::value_parser!(u64)))
    ...
    clap::Command::new("audit")
        ...
        .subcommand(
            clap::Command::new("list")
                ...
                .arg(Arg::new("limit").long("limit").default_value("20").value_parser(clap::value_parser!(usize))),
        )
        .subcommand(clap::Command::new("verify").about("Verify hash chain integrity").arg(Arg::new("session")))
        .subcommand(
            clap::Command::new("query")
                ...
                .arg(Arg::new("limit").long("limit").default_value("50").value_parser(clap::value_parser!(usize))),
        )
}
...
pub fn build_cli() -> clap::Command {
    clap::Command::new("toolshed")
        ...
        .arg(Arg::new("own-audit-trail").long("own-audit-trail").global(true).action(clap::ArgAction::SetTrue))
        .subcommand_required(true)
        .subcommand(
            clap::Command::new("list")
                .about("List tools by category or list categories")
                .arg(Arg::new("category"))
                .arg(Arg::new("health").long("health").action(clap::ArgAction::SetTrue)),
        )
        .subcommand(
        ...
        .subcommand(clap::Command::new("stop").about("Stop MCP processes").arg(Arg::new("tool")))
        .subcommand(clap::Command::new("validate").about("Validate tool manifests").arg(Arg::new("tool")))
        .subcommand(
        ...
        .subcommand(
            clap::Command::new("serve")
                ...
                .arg(Arg::new("port").long("port").default_value("8888").value_parser(clap::value_parser!(u16)))
...
fn parse_matches(matches: &ArgMatches) -> Cli {
    ...
    let command = match sub {
        ...
        "run" => Command::Run {
            tool: sub_m.get_one::<String>("tool").cloned().unwrap_or_default(),
            command: sub_m.get_one::<String>("command").cloned().unwrap_or_default(),
            full: sub_m.get_flag("full"),
            timeout: sub_m.get_one::<u64>("timeout").copied(),
            args: sub_m.get_many::<String>("args").map_or_else(Vec::new, |v| v.cloned().collect()),
        },
        "status" => Command::Status,
        "stop" => Command::Stop { tool: sub_m.get_one::<String>("tool").cloned() },
        "validate" => Command::Validate { tool: sub_m.get_one::<String>("tool").cloned() },
        "agent-prompt" => Command::AgentPrompt {
            format: sub_m.get_one::<String>("format").cloned().unwrap_or_else(|| "plain".into()),
        },
        "skill" => Command::Skill { action: parse_skill_action(sub_m) },
        "agent" => Command::Agent { action: parse_agent_action(sub_m) },
        "rule" => Command::Rule { action: parse_rule_action(sub_m) },
        "workflow" => Command::Workflow { action: parse_workflow_action(sub_m) },
        "serve" => Command::Serve {
            port: sub_m.get_one::<u16>("port").copied().unwrap_or(8888),
            category: sub_m.get_one::<String>("category").cloned(),
        },
        "audit" => Command::Audit { action: parse_audit_action(sub_m) },
        _ => unreachable!(),
    };

    Cli { own_audit_trail, command }
}

fn parse_skill_action(m: &ArgMatches) -> SkillAction {
    let (sub, sub_m) = m.subcommand().unwrap_or(("", m));
    match sub {
        "show" => SkillAction::Show { name: sub_m.get_one::<String>("name").cloned().unwrap_or_default() },
        "validate" => SkillAction::Validate { name: sub_m.get_one::<String>("name").cloned() },
        _ => SkillAction::List,
    }
}
...
fn parse_agent_action(m: &ArgMatches) -> AgentAction {
    let (sub, sub_m) = m.subcommand().unwrap_or(("", m));
    match sub {
        "show" => AgentAction::Show { name: sub_m.get_one::<String>("name").cloned().unwrap_or_default() },
        "validate" => AgentAction::Validate { name: sub_m.get_one::<String>("name").cloned() },
        _ => AgentAction::List,
    }
}
...
fn parse_rule_action(m: &ArgMatches) -> RuleAction {
    let (sub, sub_m) = m.subcommand().unwrap_or(("", m));
    match sub {
        "show" => RuleAction::Show { name: sub_m.get_one::<String>("name").cloned().unwrap_or_default() },
        "validate" => RuleAction::Validate { name: sub_m.get_one::<String>("name").cloned() },
        _ => RuleAction::List,
    }
}
...
fn parse_workflow_action(m: &ArgMatches) -> WorkflowAction {
    let (sub, sub_m) = m.subcommand().unwrap_or(("", m));
    match sub {
        "show" => WorkflowAction::Show { name: sub_m.get_one::<String>("name").cloned().unwrap_or_default() },
        "validate" => WorkflowAction::Validate { name: sub_m.get_one::<String>("name").cloned() },
...
fn parse_audit_action(m: &ArgMatches) -> AuditAction {
    let (sub, sub_m) = m.subcommand().unwrap_or(("", m));
    match sub {
        "verify" => AuditAction::Verify { session: sub_m.get_one::<String>("session").cloned() },
